...
from yolov3_tf2.models import (
    ...
    yolo_tiny_anchors, yolo_tiny_anchor_masks, YoloRecurrent
)
...
flags.DEFINE_integer('width', None, 'image width (overrides size)')
...
def main(_argv):
    ...
    model = YoloRecurrent(size, training=True, classes=FLAGS.num_classes)
    if FLAGS.tiny:
    ...
    train_dataset = train_dataset.map(lambda x, y: (
        dataset.transform_recurrent_images(x, size, anchors, anchor_masks, FLAGS.num_classes),
    ...
    val_dataset = val_dataset.map(lambda x, y: (
        dataset.transform__recurrent_images(x, size, anchors, anchor_masks, FLAGS.num_classes),
        dataset.transform_targets(y, anchors, anchor_masks, size)
    ))
